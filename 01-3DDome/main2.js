...
(function start() {
  num = 0, layer = 0, wordNum = -1
  circleArr = [], coneArr = [], liNum = 0, columnH = 0, columnNum = 0, coneNum = 0;
  theta = 0, phi = 0, r = 150;
  iNow = 0;
  oP.innerHTML = s;
  ...
  oSence.addEventListener('mousedown', function (ev) {
...
oClose2.addEventListener('click', function () {
  aBtn[0].disabled = '';
  oTexterea.value = '';
  aBtn[1].className = '';
  oText.style.transform = 'scale(0.5)';
...
function circle(aArr) {
  for (var i = 0; i < aArr.length; i++) {
    aArr[i].style.transform = `translate3D(${aArr[i].circleX}px,${aArr[i].circleY}px,${aArr[i].circleZ}px) rotateY(${aArr[i].circlePhi}rad) rotateX(${aArr[i].circleTheta}rad)`
  }
}